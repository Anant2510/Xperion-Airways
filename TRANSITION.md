...
- `EMAIL_CC` (unset) — standing cc on every email, comma list allowed; `DEMO_EMAIL_TO` also accepts a comma list (10 Sep).
...
17. **Sign in as any persona (10 Sep):** the v1 sign-in screen (`/v10`, no hash) existed but did nothing: the v1 client never sent a session id, so every request after "sign in as Sofia" was unbound and resolved to `SERVER_DEFAULT_UID` (Daniel). Now `web/app.jsx` keeps a per-tab session id in `sessionStorage` (`xp_sid`) and sends `x-session-id` on every call; sign-in posts `POST /api/auth/login {persona}` and stores the returned session; a **Traveller** picker prefills email (`<persona>@flyxperion.demo`) and password (`demo`); the header has **Switch traveller** (clears the session, returns to sign-in). Two tabs can be two travellers. The server default is unchanged, so WhatsApp pinning, the ops page and unbound MCP calls still mean Daniel. Nobody is signed in until Sign in is clicked: `/v10#app` only skips the sign-in screen when the tab already holds a session; a fresh tab always starts on the picker. The demo script's checklist should read: open `/v10`, pick Daniel, Sign in. Bundle rebuilt (`npm run build`).
18. **Clear chat is one click; a second phone for James (10 Sep):** the two-tap "Clear this chat? Yes/No" read as "not working" because the confirmation was easy to miss and clearing a thread that only held the greeting looked like nothing happened. Now one click clears (unanswered airline cards are kept), the header says "Cleared ✓" for a moment, and typing "clear the chat" does the same; v1 and v2. `WA_PHONE_MAP` accepts a pinned number without its country code (`9818447731:James`): inbound already matched on the last nine digits; outbound now borrows the country code of `WA_PAIRING_PHONE`, so James's offers go to +91 98184 47731 rather than +9818447731. Persona names in the map are case-insensitive. Remember: a phone only receives offers after it has messaged the burner once (opt-in), and `.env` changes need `pm2 restart xperion-v10 --update-env` *after* saving.
19. **Standing email copy (10 Sep):** `EMAIL_CC` (comma list) is added as cc to every email the app sends, whatever the recipient; `DEMO_EMAIL_TO` now also accepts a comma list. The outbox row records `to (cc …)`. Set `EMAIL_CC=coforgedummy@gmail.com` in the VM `.env` and `pm2 restart xperion-v10 --update-env`.
20. **Clear chat clears the notifications too (10 Sep):** "Cleared" used to keep the unanswered airline cards, so the brief and the offer stayed in the thread. Now a clear empties the thread to the greeting and calls new `POST /api/autonomy/customer/inbox/clear`, which marks the customer's `ai_inbox` rows `dismissed` (new column, added by a guarded `ALTER TABLE` on load). Dismissal is display-only: `GET /customer/inbox` hides dismissed rows, every internal lookup (`pending`, briefs, confirmations, the agent's card) still sees them, so a WhatsApp "1" after a cleared app chat still executes and its confirmation shows up as a fresh message. Verified end to end; bridge 30/30, autonomy 70/70.
