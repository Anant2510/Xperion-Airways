...
/* ── API client — every byte of personalization comes from the backend ──
   ...
   site root OR under a sub-path like /xperionportal/ behind a reverse proxy.   */
const API_BASE = (() => {
  // strip a trailing file (e.g. index.html) and trailing slash from the path
  let base = window.location.pathname.replace(/\/[^/]*$/, "");
  base = base.replace(/\/v3$/, "");   // v3 is a front-end folder; the API lives at the site root
  return base.replace(/\/$/, "");
})();
const api = {
...
const EUR = (n) => `A$${Number(n).toFixed(Number(n) % 1 === 0 ? 0 : 2)}`;
...
const Fonts = () => (
  <style>{`
    :root{ --tap-green:#A4251A; --tap-deep:#3A0F06; --tap-ink:#1F110E; --tap-mist:#F6F3F2;
      --tap-line:#E7DEDC; --tap-red:#E2354B; --tap-gold:#C9A227; --tap-amber:#E8930C;
      /* QANTAS DXP dark theme */
      --dxp-bg:#0B0A0A; --dxp-surface:#161414; --dxp-surface-2:#1F1C1C; --dxp-line:#2E2A2A;
      --dxp-text:#F6F4F4; --dxp-muted:#A39B9A; --dxp-lime:#E63E35; --dxp-lime-bright:#F2473E;
      --dxp-green:#A32B16; --dxp-green-deep:#5C180B;
      --dxp-grad:linear-gradient(135deg,#5C180B 0%,#A32B16 45%,#E63E35 130%);
      --dxp-grad-btn:linear-gradient(120deg,#7A1F0E 0%,#B23622 55%,#E8352B 120%); }
    ...
    .dxp-home{
      --tap-mist:#1F1C1C; --tap-ink:#F6F4F4; --tap-deep:#5C180B; --tap-line:#2E2A2A;
      --tap-green:#B23622;
    ...
    .dxp-home .text-gray-500{ color:#B6AFAE!important; }
    .dxp-home .text-gray-600{ color:#CBC3C3!important; }
    .dxp-home .text-gray-700{ color:#DCD7D6!important; }
    ...
    .dxp-home .dxp-primary-btn{ background:var(--dxp-grad-btn)!important; color:#1E1412!important; }
    .dxp-home .dxp-primary-btn svg{ color:#1E1412; }
    ...
    .dxp-home .dxp-pricechip{ color:var(--dxp-lime)!important; background:#1A1110!important; }
    .dxp-home .dxp-chip-green{ background:rgba(178,54,34,.16)!important; color:#E38A7B!important; }
    ...
    .dxp-home .dxp-chip-ink{ background:var(--dxp-surface-2)!important; color:#CBC3C3!important; }
    .dxp-home .dxp-chip-gold{ background:rgba(201,162,39,.16)!important; color:#E0C56A!important; }
  `}</style>
);

// Qantas lockup. Rebuilt as inline SVG so it scales crisply and themes
// for light/dark. To use the exact brand asset, drop it at public/tap-logo.png and
// replace the <svg> below with <img src="/tap-logo.png" alt="Qantas" .../>.
const TapLogo = ({ light = false, size = "text-xl" }) => (
  <div className="flex items-center gap-2">
    <svg width="38" height="28" viewBox="0 0 38 28" fill="none" aria-label="Qantas" className="shrink-0">
    ...
    <div className={`font-display font-black tracking-tight ${size} leading-none`}>
      <span style={{ color: light ? "#ffffff" : "var(--tap-ink)" }}>QANTAS</span>
      <span className={`ml-1.5 font-semibold text-[10px] tracking-[0.18em] uppercase align-middle ${light ? "text-white/80" : "text-gray-500"}`}>Australia</span>
    </div>
  </div>
);
...
const Chip = ({ children, tone = "green", className = "" }) => {
  const tones = { green:{background:"#FDECEC",color:"#E40000"}, amber:{background:"#FCF1DD",color:"#8A5A06"},
    red:{background:"#FBE4E7",color:"#A31226"}, ink:{background:"#EFEAE9",color:"#482C26"}, gold:{background:"#F7EFD6",color:"#7A6112"} };
    ...
    <span className={`relative inline-flex ${className}`}
      ...
      {open && (
        <span onClick={(e) => e.stopPropagation()}
          ...
          <span className="block font-bold mb-0.5" style={{ color: "#E39C8F" }}>Why you're seeing this</span>
          {text}
        </span>
      )}
    </span>
...
const FakeQR = ({ seed = 7, size = 120 }) => {
  ...
  return <svg width={size} height={size} viewBox="0 0 21 21" className="rounded-md bg-white p-0.5">
    {cells.map(([x, y], i) => <rect key={i} x={x} y={y} width="1" height="1" fill="#1F110E"/>)}</svg>;
};
...
/* ── LOGIN — QANTAS DXP dark split-screen ── */
...
function Login({ profile, onLogin }) {
  ...
  useEffect(() => { (async () => {
    ...
    if (r.personas?.[0]) setEmail(`${r.personas[0].id}@qantas.demo`);
  })(); }, []);
    ...
    <div className="min-h-screen flex dxp-dark">
      ...
      <div className="flex-1 flex items-center justify-center p-6 sm:p-12">
        <div className="w-full max-w-md slide-up">
          <div className="font-display font-black text-3xl tracking-tight mb-8"><span className="dxp-grad-text">QANTAS</span></div>
          ...
          <input value={email} onChange={e => setEmail(e.target.value)} onKeyDown={e => e.key === "Enter" && signIn()}
            ...
            style={{ background: "#121010", border: "1px solid var(--dxp-line)", color: "var(--dxp-text)" }} placeholder="you@qantas.demo"/>
          ...
          <div className="relative mb-5">
            <input type={showPwd ? "text" : "password"} value={pwd} onChange={e => setPwd(e.target.value)} onKeyDown={e => e.key === "Enter" && signIn()}
              ...
              style={{ background: "#121010", border: "1px solid var(--dxp-line)", color: "var(--dxp-text)" }}/>
          ...
          <button onClick={signIn} disabled={busy || !personas}
            ...
            style={{ background: "var(--dxp-grad-btn)", color: "#1E1412" }}>
          ...
          <div className="mt-5 text-xs" style={{ color: "var(--dxp-muted)" }}>
            ...
            <div className="flex flex-wrap gap-1.5 mt-2">
              {personas?.map(p => (
                <button key={p.id} onClick={() => setEmail(`${p.id}@qantas.demo`)}
                  className="px-2.5 py-1.5 rounded-lg text-[11px] transition-colors hover:brightness-125"
                  style={{ background: "var(--dxp-surface-2)", color: email.startsWith(p.id) ? "var(--dxp-lime)" : "var(--dxp-muted)", border: "1px solid var(--dxp-line)" }}>{p.id}@qantas.demo</button>
              ))}
            </div>
          </div>
  ...
  return (
    <div className="min-h-screen flex" style={{ background: "var(--tap-mist)" }}>
      <div className="hidden lg:flex flex-col justify-between w-[44%] p-12 text-white relative overflow-hidden"
        style={{ background: "linear-gradient(160deg, var(--tap-deep) 0%, #5A180A 70%, var(--tap-green) 130%)" }}>
        <TapLogo light/>
        <div>
          <div className="text-xs font-bold tracking-[0.25em] uppercase text-white/60 mb-4">Qantas Frequent Flyer · Digital channel</div>
      ...
      <div className="flex-1 flex items-center justify-center p-6">
        <div className="w-full max-w-md slide-up">
          ...
          <p className="text-sm text-gray-500 mb-8">Sign in to Qantas Frequent Flyer</p>
...
/* Qantas-style segmented quick-book widget — fully functional, DB-backed.
   Origin + Destination come from the real 100-route network (/api/routes):
   pick any origin, and the destination list shows every city Qantas actually
...
function Home({ profile, destinations, go, openAssistant, toast, bookDestination, bookUsual, resumeJourney, startFresh, openExtras, openExpress }) {
  ...
  const navItem = (label, onClick, active) => (
    <button onClick={onClick} className="px-1 pb-1 text-sm font-semibold relative whitespace-nowrap" style={{ color: active ? "var(--tap-ink)" : "#6B5E5B" }}>
  ...
  const toggle = (on, set, size = "lg") => (
    <span onClick={set} className={`${size === "lg" ? "w-10 h-6" : "w-9 h-5"} rounded-full p-0.5 transition-colors cursor-pointer shrink-0`} style={{ background: on ? "var(--tap-green)" : "#D5CDCB" }}>
  ...
  return (
    <div style={{ background: "var(--tap-mist)" }}>
      {/* ── Top nav (light, mirrors qantas.com) ── */}
      <header className="sticky top-0 z-40 bg-white border-b" style={{ borderColor: "var(--tap-line)" }}>
        <div className="max-w-[1180px] mx-auto px-5 h-16 flex items-center gap-6">
          ...
          <nav className="hidden lg:flex items-center gap-6">
            ...
            {navItem("Qantas Frequent Flyer", () => go("miles"), false)}
          </nav>
          <div className="flex items-center gap-3 ml-auto shrink-0">
            ...
            <span className="hidden md:flex items-center gap-1 text-xs font-semibold text-gray-600"><Globe size={14}/> AU · AUD</span>
          ...
          <div className="grid lg:grid-cols-[1.35fr_1fr] gap-8 items-start">
            ...
            <div>
              <div className="inline-flex flex-wrap items-center gap-2.5 rounded-full pl-1.5 pr-4 py-1.5 mb-3" style={{ background: "rgba(0,0,0,.22)", backdropFilter: "blur(6px)" }}>
                ...
                <span className="text-[13px] font-semibold" style={{ color: "#FFE7B0" }}>{u.miles.toLocaleString()} Qantas Points</span>
              </div>
              ...
              {offer ? (
                <div className="mt-3 rounded-2xl px-4 py-3 max-w-md" style={{ background: "rgba(0,0,0,.26)", backdropFilter: "blur(6px)", border: "1px solid rgba(255,255,255,.18)" }}>
                  ...
                  <div className="flex items-end gap-2 flex-wrap">
                    ...
                    {offer.discountPct > 0 && <span className="text-[11px] font-bold px-1.5 py-0.5 rounded" style={{ background: "var(--tap-green)", color: "#1E1412" }}>−{offer.discountPct}%</span>}
                  </div>
            ...
            <div className="space-y-4">
              {upF ? (
                <div className="rounded-2xl p-4 text-white shadow-xl" style={{ background: "linear-gradient(180deg,#2A130E,#1C0D0A)" }}>
                  ...
                  {upcoming.checked_in ? (
                    ...
                    <div className="flex items-center justify-between mt-3">
                      ...
                      <button onClick={() => go("checkin")} className="text-[13px] font-bold px-3 py-1.5 rounded-lg inline-flex items-center gap-1.5" style={{ background: "var(--tap-green)", color: "#1E1412" }}>Check in now <ArrowRight size={14}/></button>
                    </div>
                  ) : (
                    <>
                      ...
                      <div className="flex items-center justify-between mt-2.5">
                        ...
                        <button onClick={() => { toast(`${u.tier} upgrade`, `Front-row seat for ${EUR(upgradePrice)} added to ${upcoming.pnr}.`); go("manage"); }} className="text-[13px] font-bold px-3 py-1.5 rounded-lg inline-flex items-center gap-1.5" style={{ background: "var(--tap-green)", color: "#1E1412" }}>Add upgrade · {EUR(upgradePrice)}</button>
                      </div>
                    </>
                  )}
                </div>
                ...
                <div className="rounded-2xl p-4 text-white shadow-xl" style={{ background: "linear-gradient(180deg,#2A130E,#1C0D0A)" }}>
                  ...
                  <button onClick={openExpress} className="w-full mt-3 inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl font-bold" style={{ background: "var(--tap-green)", color: "#1E1412" }}><Zap size={15}/> Express checkout</button>
              ...
              {TABS.map(t => {
                ...
                return (
                  <button key={t} disabled={!enabled}
                    ...
                    style={active ? { background: "#F7E9E7", color: "var(--tap-deep)" } : enabled ? { color: "#6B5E5B" } : { color: "#CCC4C3" }}>{t}</button>
                );
              })}
            </div>
            <div className="flex items-center justify-between gap-3 mb-3 px-3 py-2 rounded-xl" style={{ background: "#F8ECEA" }}>
          ...
          {destinations.slice(0, 1).map((d) => {
            ...
            return (
              <button key={d.code} onClick={() => bookDestination(d)} className="lg:row-span-2 relative rounded-3xl overflow-hidden group text-left min-h-[300px] lg:min-h-[420px]">
                ...
                <div className="absolute inset-0" style={{ background: "linear-gradient(180deg, rgba(18,10,8,.05) 30%, rgba(18,10,8,.82) 100%)" }}/>
            ...
            {destinations.slice(1, 5).map((d) => {
              ...
              return (
                <button key={d.code} onClick={() => bookDestination(d)} className="relative rounded-3xl overflow-hidden group text-left min-h-[200px]">
                  <CityImg code={d.code} alt={d.city} className="absolute inset-0 w-full h-full object-cover transition-transform duration-500 group-hover:scale-105"/>
                  <div className="absolute inset-0" style={{ background: "linear-gradient(180deg, rgba(18,10,8,.1) 25%, rgba(18,10,8,.8) 100%)" }}/>
                  <span className="absolute top-3 right-3 text-[10px] font-bold px-2.5 py-1 rounded-full text-white" style={{ background: booked > 0 ? "var(--tap-green)" : "rgba(18,10,8,.6)", backdropFilter: "blur(4px)" }}>{booked > 0 ? `Booked ${booked}×` : (d.tag || "Popular")}</span>
        ...
        {rec && rec.package && (
          <div className="mt-10">
            <div className="flex items-center gap-2 mb-4">
              ...
              <Why text={`Derived from your Qantas Frequent Flyer co-branded card spend pattern. ${rec.rationale} Bundles event ticket + hotel + return flight.`}/>
            </div>
            <div className="bg-white rounded-3xl overflow-hidden border grid md:grid-cols-2" style={{ borderColor: "var(--tap-line)" }}>
              <div className="relative min-h-[240px]">
                ...
                <div className="absolute inset-0" style={{ background: "linear-gradient(180deg, rgba(18,10,8,.08), rgba(18,10,8,.55))" }}/>
              ...
              <div className="p-6">
                ...
                {rec.package.addon && (
                  <div className="flex items-start gap-2.5 p-3 rounded-xl mb-4" style={{ background: "#FAF2F0", border: "1px solid #EBC4BD" }}>
      ...
      <button onClick={openAssistant} className="fixed bottom-6 right-6 z-30 w-14 h-14 rounded-full shadow-xl flex items-center justify-center text-white" style={{ background: "var(--tap-red)" }} aria-label="Open Qantas AI Assistant"><Sparkles size={24}/></button>
    </div>
  );
}
...
function SeatMap({ flight, seat, setSeat, go, toast, profile }) {
  ...
  const CABINS = [
    { id: "business", label: "Business", rows: [1, 2, 3], cols: ["A", "C", "D", "F"], aisleAfter: 1, price: 0, freeFor: ["Platinum"], note: "Lie-flat · lounge included", color: "#3A0F06" },
    { id: "premium", label: "Premium Economy", rows: [4, 5, 6, 7], cols: ["A", "B", "C", "D", "E", "F"], aisleAfter: 2, price: 18, freeFor: ["Platinum", "Gold"], note: "Extra legroom · priority boarding", color: "#5A180A" },
    { id: "economy", label: "Economy", rows: Array.from({ length: 23 }, (_, i) => i + 8), cols: ["A", "B", "C", "D", "E", "F"], aisleAfter: 2, price: 0, premiumRows: { min: 8, max: 10, price: 8 }, note: "Standard cabin · front rows are extra-legroom", color: "#A31B00" },
  ];
  ...
  const pick = (s, row) => {
    ...
    toast("Seat selected", `${s} · ${classOf(s)}${p ? ` · A$${p}` : " · included"}`);
  };
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 pb-24 pt-8">
      ...
      {rec && recSeat && (
        <Card className="p-4 mb-5 flex items-center justify-between gap-3 flex-wrap" style={{ borderColor: "var(--tap-green)", background: "#FEFBFB" }}>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-gray-500">{rec.reason} · {recPrice ? `A$${recPrice}` : `included with ${tier}`}</div>
            </div>
          </div>
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        <Card className="lg:col-span-2 p-5 overflow-x-auto">
                      ...
                      return (
                        <React.Fragment key={s}>
                          <button onClick={() => pick(s, row)} disabled={isTaken}
                            title={isTaken ? "Occupied" : `Seat ${s} · ${cabin.label}${price ? ` · A$${price}` : " · included"}`}
                            className={`${w} h-7 rounded-md text-[9px] font-bold flex items-center justify-center transition-all`}
                            style={{
                              background: isSel ? "var(--tap-green)" : isTaken ? "#e5e7eb" : isRec ? "#FAD8D1" : "#fff",
                              color: isSel ? "#fff" : isTaken ? "#9ca3af" : "var(--tap-ink)",
                              border: `1.5px solid ${isSel ? "var(--tap-green)" : isRec ? "var(--tap-green)" : cabin.id === "business" ? "#D7C0BC" : "var(--tap-line)"}`,
          ...
          <div className="flex items-center justify-center gap-4 mt-4 text-[11px] text-gray-500 flex-wrap">
            ...
            <span className="flex items-center gap-1"><span className="w-3 h-3 rounded" style={{ background: "#FAD8D1", border: "1px solid var(--tap-green)" }}/> Recommended</span>
          ...
          <Card className="p-5 sticky top-20">
            ...
            <div className="flex items-center gap-3 mb-3">
              ...
              <div className="text-sm text-gray-600">
                ...
                <br/><span className="text-xs text-gray-400">{(() => { const sc = seat || recSeat; if (!sc) return ""; const p = seatPriceFor(parseInt(sc)); return p ? `A$${p}` : `Included with ${tier}`; })()}</span>
              </div>
            </div>
      ...
      <div className="space-y-3 mb-6">
        {voucher && (
          <Card className="p-4 flex items-center gap-4">
            ...
            <button onClick={()=>setUseVoucher(!useVoucher)} role="switch" aria-checked={useVoucher}
              className="w-11 h-6 rounded-full relative transition-colors shrink-0" style={{background: useVoucher ? "var(--tap-green)" : "#E0D8D8"}}>
        ...
        <Card className="p-4">
          <div className="flex items-center gap-4 mb-3">
            ...
            <div className="flex-1">
              <div className="text-sm font-bold" style={{color:"var(--tap-ink)"}}>Frequent Flyer balance — {u.miles.toLocaleString()} miles</div>
          ...
          <input type="range" min={0} max={maxMiles} step={500} value={Math.min(milesUsed, maxMiles)}
            onChange={(e)=>setMilesUsed(+e.target.value)} className="w-full accent-[#A31B00]"/>
...
function ExpressCheckout({ profile, flight, ancillaries, items, seat, onPaid, toast, go }) {
  ...
  const retNo = pat.usualBackNo || ("XP" + ((parseInt((out.flight_no || "QF1923").replace(/\D/g, "")) || 1923) + 19));
        ...
        <div className="space-y-4">
          ...
          <Section title="Seat selection" action="Change seat" onAction={() => go("seatmap")}>
            ...
            <div className="flex items-center justify-between text-sm">
              ...
              <span className="text-[11px] font-bold px-2 py-0.5 rounded-full" style={{ background: "#FDECEC", color: "#E40000" }}>FREE · {(u.tier || "GOLD").toUpperCase()}</span>
            </div>
          </Section>
          ...
          <Section title="Baggage &amp; extras" action="Manage" onAction={() => go("basket")}>
            {[
              ...
              <div key={i} className="flex items-center justify-between py-1.5 text-sm">
                ...
                {x.free ? <span className="text-[11px] font-bold px-2 py-0.5 rounded-full" style={{ background: "#FDECEC", color: "#E40000" }}>{x.sub.startsWith("Included in") ? "INCLUDED" : `FREE · ${(u.tier || "GOLD").toUpperCase()}`}</span> : <span className="font-bold" style={{ color: "var(--tap-ink)" }}>{EUR(x.price)}</span>}
              </div>
            ))}
          </Section>
          ...
          <Section title="Payment method" action="+ Change" onAction={() => toast("Payment", "Use your saved card, miles, MB WAY, Apple Pay or PayPal at payment.")}>
            <div className="flex items-center justify-between">
              <div className="text-sm font-bold" style={{ color: "var(--tap-ink)" }}>Saved card · {MASKED_PAN}</div>
              <span className="inline-flex items-center gap-1 text-[11px] font-bold px-2 py-0.5 rounded-full" style={{ background: "#FDECEC", color: "#E40000" }}>🔒 Encrypted</span>
            </div>
          ...
          <label className="flex items-start gap-2 text-sm text-gray-600 cursor-pointer px-1">
            <input type="checkbox" checked={accept} onChange={(e) => setAccept(e.target.checked)} className="mt-0.5 accent-[#A4251A]"/>
...
function Confirmed({ profile, flight, receipt, go }) {
  ...
  const paidLabel = receipt.cardVal > 0 ? "your saved card" : (receipt.milesVal > 0 ? "Xperion Miles" : "Voucher");
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 pb-24 pt-8">
      <div className="flex items-center gap-3 mb-6">
        <div className="w-9 h-9 rounded-full flex items-center justify-center shrink-0 slide-up" style={{ background: "#FDECEC" }}><CheckCircle2 size={20} style={{ color: "var(--tap-green)" }}/></div>
        <div>
      ...
      <div className="grid lg:grid-cols-[1fr_320px] gap-5 items-start">
        <div className="space-y-5">
          <Card className="p-5">
            ...
            <div className="rounded-2xl p-4" style={{ background: "#F7E3E1" }}>
        ...
        <div className="lg:sticky lg:top-20 space-y-4">
          <Card className="p-5">
            ...
            <div className="rounded-xl px-3 py-2.5 mt-3 text-[12px]" style={{ background: "#F7E3E1" }}>
              <div className="font-bold" style={{ color: "#E40000" }}>✦ You earned {milesEarned.toLocaleString()} miles</div>
          ...
          <Card className="p-4 text-[12px] text-gray-500 space-y-1.5">
            ...
            <div className="flex items-center gap-2"><Sparkles size={13} style={{ color: "var(--tap-green)" }}/> 24/7 Qantas Care — chat anytime</div>
          </Card>
    ...
    <div className="max-w-3xl mx-auto px-4 pb-24 pt-8">
              ...
              {loadingRec ? (
                ...
                <>
                  ...
                  {!rebooked ? (
                    ...
                    <div className="p-3 rounded-xl flex items-center gap-2 text-sm font-semibold" style={{background:"#FDECEC", color:"#E40000"}}>
      ...
      <Card className="p-4 mb-5 flex items-center gap-4">
        ...
        <button onClick={toggleCI} role="switch" aria-checked={autoCI}
          className="w-11 h-6 rounded-full relative transition-colors shrink-0" style={{background: autoCI ? "var(--tap-green)" : "#E0D8D8"}}>
          <span className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-all ${autoCI?"left-[22px]":"left-0.5"}`}/>
        </button>
      </Card>

      <Card className="p-4 flex flex-wrap items-center gap-3" style={{background:"#FEFBFB"}}>
        <div className="text-xs text-gray-500 flex-1 min-w-[200px]"><b>Demo control:</b> trigger a live disruption — the ops event hits the DB, Qantas AI writes the recovery, and the notification email goes out in real time.</div>
        ...
        <GhostBtn onClick={openAssistant}><MessageCircle size={15}/> Ask Qantas AI</GhostBtn>
      </Card>

      {showPass && passBooking && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{background:"rgba(30,10,6,.6)"}} onClick={()=>setShowPass(false)}>
  ...
  const SrcBtn = ({ id, label, sub, icon }) => {
    ...
    return (
      <button onClick={() => pick(id)} disabled={busy} className="flex-1 text-left rounded-2xl border-2 p-3 transition disabled:opacity-60"
        style={{ borderColor: on ? (id === "adobe" ? "#FA0F00" : "var(--tap-green)") : "var(--tap-line)", background: on ? (id === "adobe" ? "#FFF1F0" : "#FDECEC") : "#fff" }}>
        <div className="flex items-center gap-2">
    ...
    <Card className="p-4 mb-5" style={{ borderColor: "var(--tap-green)" }}>
      <div className="flex items-center justify-between gap-3 flex-wrap mb-3">
        <div className="flex items-center gap-2.5">
          <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0" style={{ background: "#FDECEC" }}><Database size={18} style={{ color: "var(--tap-green)" }}/></div>
          <div>
      ...
      {ev && ev.configured && ev.recent && ev.recent.length > 0 && (
        <div className="rounded-xl border p-3 mb-4" style={{ borderColor: "var(--tap-green)", background: "#FEFBFB" }}>
            ...
            {ev.recent.slice(0, 6).map((e, i) => (
              <div key={i} className="flex items-center justify-between gap-2 text-[10px]">
                <div className="flex items-center gap-2 min-w-0">
                  <span className="px-1.5 py-0.5 rounded-full font-semibold shrink-0" style={{ background: e.ok ? "#FDECEC" : "#FDE5E3", color: e.ok ? "var(--tap-deep)" : "#B11209" }}>{e.eventType || e.stage}</span>
      ...
      <div className="rounded-xl border p-3 mb-4 overflow-x-auto" style={{ borderColor: "var(--tap-line)", background: "#FEFBFB" }}>
        ...
        <div className="flex items-center gap-1.5 min-w-[640px] text-center">
          {[
            ...
            { t: "Qantas AI", s: "agent tools", ic: <Sparkles size={14}/> },
          ].map((n, i) => (
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <button onClick={() => setOpen(o => !o)} className="flex items-center gap-2.5 text-left">
          <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0"
            style={{ background: result ? (allGreen ? "#FDECEC" : "#FBE4E7") : "var(--tap-mist)" }}>
    ...
    <div className="rounded-2xl border p-5 mb-5" style={{ borderColor: "var(--tap-line)", background: "var(--tap-mist)" }}>
      ...
      <div className="grid md:grid-cols-3 gap-4 items-stretch">
        ...
        <div className="rounded-xl border p-4" style={{ borderColor: "var(--tap-line)", background: "var(--dxp-surface)" }}>
          ...
          <div className="text-sm font-bold mb-3" style={{ color: "var(--tap-ink)" }}>Qantas Frequent Flyer co-branded card <span className="text-gray-400 font-semibold">•••• ••••</span></div>
          <div className="space-y-2">
            {(rec.categories || []).map((c, i) => (
              <div key={i}>
                ...
                <div className="h-1.5 rounded-full" style={{ background: "var(--dxp-surface-2)" }}><div className="h-full rounded-full" style={{ width: `${(c.share / maxShare) * 100}%`, background: i === 0 ? "var(--dxp-lime)" : "#7A625E" }}/></div>
              </div>
            ))}
          </div>
        </div>
        {/* 2 — derived affinity */}
        <div className="rounded-xl border p-4 flex flex-col justify-center items-center text-center" style={{ borderColor: "rgba(230,62,53,.35)", background: "#1A1110" }}>
        ...
        <div className="rounded-xl border p-4" style={{ borderColor: "var(--tap-line)", background: "var(--dxp-surface)" }}>
          ...
          <div className="space-y-1 text-[11px]" style={{ color: "#CBC3C3" }}>
...
function Console({ toast }) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 pb-24 pt-8">
      ...
      {openEmail && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{background:"rgba(30,10,6,.6)"}} onClick={()=>setOpenEmail(null)}>
...
/* ── ASSISTANT — Qantas AI concierge via backend ── */
      ...
      {cards.map((c, i) => {
        ...
        if (c.type === "package") return (
          <div key={i} className="rounded-2xl border overflow-hidden" style={{borderColor:"var(--dxp-line)", background:"var(--dxp-surface)"}}>
            <div className="p-3" style={{background:"#1A1110"}}>
            ...
            <div className="p-3 space-y-1 text-xs" style={{color:"#CBC3C3"}}>
          ...
          <div key={i} className="bg-white border rounded-2xl p-3" style={{borderColor:"var(--tap-line)"}}>
            ...
            <div className="grid grid-cols-2 gap-2 mt-2">
              <div className="rounded-xl p-2" style={{background:"var(--tap-mist)"}}>
                <div className="text-[10px] text-gray-400">Frequent Flyer</div>
...
function Assistant({ open, onClose, screen, profile, onCommand, onSelectFlight }) {
  ...
  if (!open) return null;
  const chips = [`Best time to visit ${d1}`, `Flights under A$500 to ${d2}`, `${d2} in October?`];
  return (
      ...
      <div className="w-full h-full sm:w-[390px] sm:h-[600px] sm:max-h-[82vh] bg-white sm:rounded-3xl shadow-2xl flex flex-col overflow-hidden slide-up" onClick={(e) => e.stopPropagation()} style={{ border: "1px solid var(--tap-line)" }}>
        ...
        <div className="px-4 py-3.5 flex items-center justify-between" style={{ background: "linear-gradient(120deg,#E2354B 0%,#C01030 100%)" }}>
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <div className="text-white font-extrabold text-[15px] leading-tight">Qantas AI Assistant</div>
              <div className="text-white/85 text-[11px] flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full" style={{ background: "#DE604A" }}/> Online</div>
            </div>
          </div>
          <button onClick={onClose} className="text-white/80 hover:text-white" aria-label="Close assistant"><X size={20}/></button>
        </div>
        {/* messages */}
        <div className="flex-1 overflow-y-auto p-4 space-y-3" style={{ background: "#FBFAFA" }}>
          {msgs.map((m, i) => (
            <div key={i} className="space-y-2">
              {m.content && <div className={`max-w-[88%] px-4 py-2.5 rounded-2xl text-[15px] leading-relaxed whitespace-pre-wrap ${m.role === "user" ? "ml-auto text-white rounded-br-md" : "rounded-bl-md"}`}
                style={m.role === "user" ? { background: "var(--tap-green)" } : { background: "#F1EEEE", color: "var(--tap-ink)" }}>{m.content}</div>}
          ...
          {busy && <div className="px-4 py-2.5 rounded-2xl rounded-bl-md w-fit" style={{ background: "#F1EEEE" }}><Loader2 className="animate-spin" size={15} style={{ color: "var(--tap-green)" }}/></div>}
          <div ref={endRef}/>
        </div>
        {/* chips + input */}
        <div className="p-3 border-t bg-white" style={{ borderColor: "var(--tap-line)" }}>
          ...
          <div className="flex flex-wrap gap-2 mb-2.5">
            {chips.map(s => (
              <button key={s} onClick={() => send(s)} className="text-[12px] font-semibold px-3 py-1.5 rounded-full" style={{ background: "#F3F1F1", color: "var(--tap-deep)" }}>{s}</button>
            ))}
          </div>
          <div className="flex items-center gap-2 rounded-full pl-4 pr-1.5 py-1.5" style={{ background: "#F3F1F1" }}>
...
function SearchScreen({ origin, setOrigin, dest, setDest, date, setDate, onSearch, results, searching, selectFlight, routes, suggested, prefilledReason }) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 pb-24 pt-8">
      ...
      {prefilledReason && (
        <Card className="p-3 mb-4 flex items-center gap-2 slide-up" style={{ background: "#FEFBFB", borderColor: "var(--tap-green)" }}>
...
/* ── APP SHELL ── */
// Nav mirrors the real qantas.com labels, with our cleaner styling.
const NAV = [
  { id: "home",   label: "Plan a trip" },
  { id: "manage", label: "My flights" },
  { id: "checkin",label: "Check-in" },
  { id: "miles",  label: "Qantas Frequent Flyer" },
  { id: "help",   label: "Help" },
];
...
function MilesGo({ profile, go }) {
  ...
  const PERKS = {
    Silver: [
      ...
      "One free checked bag on every Qantas flight",
      "Seat selection and 24h fare lock included",
      "Group B priority boarding",
      "25% bonus miles on Qantas flights",
    ],
    Gold: [
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 pb-24 pt-8">
      <div className="flex items-center gap-3 mb-1 flex-wrap">
        <h1 className="font-display font-black text-3xl" style={{ color: "var(--tap-ink)" }}>Qantas Frequent Flyer</h1>
      ...
      <div className="grid sm:grid-cols-2 gap-4 mb-5">
        <Card className="p-5">
          ...
          <div className="flex items-center justify-between py-1.5 border-b" style={{ borderColor: "var(--tap-line)" }}>
            <span className="text-sm text-gray-600">Travel voucher</span><span className="font-bold text-sm" style={{ color: "var(--tap-ink)" }}>A${voucher?.amount ?? 0}</span>
          </div>
          <div className="flex items-center justify-between py-1.5 border-b" style={{ borderColor: "var(--tap-line)" }}>
          ...
          <div className="flex items-center justify-between py-1.5">
            <span className="text-sm text-gray-600">Miles value (approx)</span><span className="font-bold text-sm" style={{ color: "var(--tap-ink)" }}>A${Math.round(u.miles / 1000 * 3).toLocaleString()}</span>
          </div>
        </Card>
        <Card className="p-5">
...
function Help({ openAssistant }) {
  const faqs = [
    { q: "How do I change or cancel a booking?", a: "Open My trips, or just ask Qantas AI — it can rebook, change seats, or cancel with an instant refund in one step." },
    ...
    { q: "What can I spend my miles on?", a: "Flights, seat upgrades, extra bags and lounge passes. Your current balance and its approximate value are on the Frequent Flyer page." },
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 pb-24 pt-8">
      ...
      <p className="text-sm text-gray-500 mb-6">Quick answers — or ask Qantas AI anything and it'll handle it for you.</p>

      <Card className="p-5 mb-5 flex items-center justify-between gap-3 flex-wrap" style={{ background: "#FEFBFB", borderColor: "var(--tap-green)" }}>
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="font-bold text-sm" style={{ color: "var(--tap-ink)" }}>Ask Qantas AI</div>
            <div className="text-xs text-gray-500">Find flights, book, check in or cancel — right in the chat.</div>
          </div>
        </div>
        <PrimaryBtn onClick={openAssistant} className="!py-2.5"><MessageCircle size={15}/> Open Qantas AI</PrimaryBtn>
      </Card>
...
function App() {
  ...
  return (
    <div className="min-h-screen" style={{background: "var(--tap-mist)"}}>
      ...
      <header className="sticky top-0 z-40 border-b" style={{background: "#fff", borderColor: "var(--tap-line)"}}>
        <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between gap-4">
          ...
          <nav className="hidden md:flex items-center gap-1">
            {NAV.map((s)=>(
              <button key={s.id} onClick={()=>go(s.id)}
                ...
                style={s.id===activeNav?{background:"var(--tap-green)"}:(onHome?{color:"#CBC3C3"}:{})}>
                {s.label}
              </button>
            ))}
          </nav>
          <div className="flex items-center gap-2.5 shrink-0">
            ...
            <div className="w-9 h-9 rounded-full flex items-center justify-center font-display font-extrabold text-sm" style={{background: onHome ? "var(--dxp-lime)" : "var(--tap-deep)", color: onHome ? "#1E1412" : "#fff"}}>{profile.user.full_name.split(" ").map(w=>w[0]).slice(0,2).join("")}</div>
      ...
      {/* Inner screens now use the light Qantas theme, matching Home */}
      <div>
      ...
      <footer className="border-t py-4 text-center text-[11px]" style={{borderColor: "var(--tap-line)", color: "#A39794"}}>
